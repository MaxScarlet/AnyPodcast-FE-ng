...
export class FileUploadComponent {
  ...
  onUploadInit(): void {
    ...
    let upload: Upload = { FileName: this.selectedFile.name, User: this.user, TotalParts: totalParts, Size: this.selectedFile.size };
    ...
    this.http.post<any>(`${this.baseUrl}/init`, upload, options).subscribe(
      (upload: Upload) => {
        ...
        for (let partNumber = 1; partNumber <= totalParts; partNumber++) {
          ...
          this.http.put(part.PresignedUrl, filePart, { observe: 'response', reportProgress: true }).subscribe(
            response => {
              if (response.status === 200 || response.status === 204) {
                ...
                console.error(`Failed to upload part: HTTP ${response.status}`);
              }
            },
            error => {
